...
- [x] Port edge-panel toggles from `functions/edges.R` into `src/ui/edge.ts` (done with the Edge panel above: `handleEdgeWidthByWeightCheckbox`/`handleEdgeDirectionCheckbox` slider show/hide). `updateSelectedEdgesView` remains — it renders the Selected Edges data table; port it with the View Data panel, then delete `edges.R`.
