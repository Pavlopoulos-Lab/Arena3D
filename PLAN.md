...
- [ ] Delete `www/js/object_actions/` — **partially done**: `screen.js`, `labels.js`, `themes.js`, `right_click_menu.js` deleted (fully ported). Kept as living spec for their unported remainders: `layout.js` (predefined/star/cube layouts), `network.js` (`importNetwork` session JSON), `node.js`/`edge.js`/`layer.js`/`canvas_controls.js` (Phase 13 DOM: sliders, checkbox group, channel lists, descr div, nav table).
